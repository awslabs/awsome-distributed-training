"""
Nemotron 3 Super — GRPO/DAPO Reinforcement Learning on Kubernetes with NeMo-Run
...
training for Nemotron 3 Super on a Kubernetes cluster (e.g. Amazon EKS).

Architecture: LatentMoE + Mamba-2 + Attention + MTP hybrid.
Super has 120B total / 12B active parameters with 512+1 experts.
...
Supported GPU types (GRPO requires more resources than SFT):
    - B200 (p6-B200)       — 1-2 nodes
    - H200 (p5en.48xlarge) — 2+ nodes
    - H100 (p5.48xlarge)   — 2+ nodes (FP8 required)
    - A100 (p4de.24xlarge) — Not recommended for Super
...
# ---------------------------------------------------------------------------
DEFAULT_GRPO_CONTAINER_IMAGE = "aws-nemotron3-super-grpo:v0.5.0"
...
def get_parser() -> argparse.ArgumentParser:
    """Build the argument parser for Nemotron 3 Super GRPO on Kubernetes."""

    parser = argparse.ArgumentParser(
        description="Nemotron 3 Super GRPO/DAPO on Kubernetes (NeMo-Run + SkyPilot)",
    ...
    rollout.add_argument(
        "--rollout_batch_size", type=int, default=256,
        help="Number of prompts per rollout batch (reduced vs Nano due to model size)",
    )
    rollout.add_argument(
    ...
    # -- Parallelism ----------------------------------------------------------
    # Super defaults: TP=2 for 12B active params, EP=4 for 512-expert LatentMoE
    par = parser.add_argument_group("Parallelism")
    par.add_argument("--tp", type=int, default=2, help="Tensor parallelism for actor (default 2 for Super)")
    par.add_argument("--ep", type=int, default=4, help="Expert parallelism (Super has 512+1 experts)")
...
def build_grpo_config(args) -> dict:
    ...
    model_path = args.model_path or f"{args.pvc_mount}/models/nemotron3-super-hf"
    ...
    config = {
        ...
        "training": {
            ...
            "save_interval": 50,
            "log_interval": 1,
            "output_dir": f"{args.pvc_mount}/results/nemotron3-super-{args.algorithm}",
        },
    }
...
def build_nemorl_command(args, config_path: str) -> list[str]:
    ...
    cmd = [
        ...
        "--output-dir", f"{args.pvc_mount}/results/nemotron3-super-{args.algorithm}",
...
if __name__ == "__main__":
    ...
    output_dir = f"{pvc_mount}/results/nemotron3-super-{args.algorithm}"
    config_dir = f"{pvc_mount}/configs"

    exp_name = f"nemotron3-super-{args.algorithm}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    print()
    print("Note: Super (120B) requires 2+ nodes for GRPO.")
    print("      FP8 model variant is recommended on Hopper GPUs.")
