"""
Nemotron 3 Nano — GRPO/DAPO Reinforcement Learning with NeMo RL on Slurm

This script launches GRPO (Group Relative Policy Optimization) or DAPO
(Direct Advantage Policy Optimization) training for Nemotron 3 Nano
using NeMo RL and NeMo Gym on a Slurm cluster.

Usage:
    # GRPO with math reasoning environment
    python run_grpo.py \
        --container_image ~/aws-nemotron3-nano-grpo.sqsh \
...
Supported instance types:
    - p6-B200 (8x B200 180GB) — 1 node
    - p5en.48xlarge (8x H200 141GB) — 1 node
    - p5.48xlarge (8x H100 80GB) — 1-2 nodes
    - p4de.24xlarge (8x A100 80GB) — 2+ nodes
"""
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Nemotron 3 Nano GRPO/DAPO on Slurm"
    )
    ...
    parser.add_argument("--nodes", type=int, default=2,
                        help="Number of nodes (recommend 2+ for RL)")
    ...
    parser.add_argument("--model_path", type=str, default="/fsx/models/nemotron3-nano-hf",
                        help="Local path to downloaded HF model")
    ...
    parser.add_argument("--tp", type=int, default=1,
                        help="Tensor parallelism for actor")
    ...
    parser.add_argument("--output_dir", type=str,
                        default="/fsx/results/nemotron3-nano-grpo",
...
if __name__ == "__main__":
    ...
    exp_name = f"nemotron3-nano-{args.algorithm}-{suffix}"
